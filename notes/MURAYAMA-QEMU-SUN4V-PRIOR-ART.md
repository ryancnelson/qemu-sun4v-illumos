...
- [x] Clone and pin all five source repositories and record full commit IDs.
- [x] Build the `sun4v` QEMU branch from source on Biggie; do not initially
  run the distributed executable.  The pinned source build completed with
  `BUILD_RC=0` and its firmware-only smoke test reached `{0} ok`.
