...
| In progress | P0 | Biggie | Run `term4code` | Boot the exact published `ppp-injected-v2-20260825` installer with private units 100/103 and a featureless, cleanly exported `tink` pool on 60 GiB unit 104 | Launch/NVRAM/hSIMD gates passed; missing `/.cdrom` caused the initial lofi failure; in-place recovery proved writable RAM root and mounted exact unit-103 media, `/usr`, and `/mnt/misc`; next gate is read-only unit-104 identity and `tink` import without upgrade |
