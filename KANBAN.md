...
| In progress | P0 | Biggie | Run `term4code` | Boot the exact published `ppp-injected-v2-20260825` installer with private units 100/103 and a featureless, cleanly exported `tink` pool on 60 GiB unit 104 | Launch/NVRAM/hSIMD gates passed; current media-lofi gate failed at `/usr`, and the documented maintenance prompt awaits its single console writer |
