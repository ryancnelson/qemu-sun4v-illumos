...
| Blocked | P0 | Biggie | Run `term4code` | Preserve the exact published installer run and its imported featureless `tink` pool after the first dataset mutation panicked in hSIMD | Unit-104 identity/import and `tink@empty-imported` passed; `zfs create tink/source` issued a `0x24000` request and hit `sz <= 128*1024`; console is preserved at KMDB, run is not closed, and recovery requires explicit authorization |
