...
| In progress | P0 | Biggie | Trial `term4code-02` | Fortify the completed unit104 installation from isolated live media, then cold boot without KMDB | Media mounts, bounded ZFS 256 KiB write/sync/read, unit101 echo, PPP, outbound ping, and installation PASS; installed `-k -v` boot repeatedly stops in KMDB at `page_list_add+0x9c`, with no panic; QEMU retired through monitor and evidence preserved |
