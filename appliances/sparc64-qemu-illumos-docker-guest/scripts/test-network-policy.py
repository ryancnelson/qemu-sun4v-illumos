...
cases = [
    ...
    ('tabs and comments', 'nameserver 8.8.8.8\n', 'hosts:\tfiles dns # comment\nipnodes: files dns\n', True),
    ('old resolver', 'nameserver 10.0.5.1\n', 'hosts: files dns\nipnodes: files dns\n', False),
    ('missing dns', 'nameserver 8.8.8.8\n', 'hosts: files\nipnodes: files dns\n', False),
    ('wrong order', 'nameserver 8.8.8.8\n', 'hosts: dns files\nipnodes: files dns\n', False),
    ('missing ipnodes', 'nameserver 8.8.8.8\n', 'hosts: files dns\n', False),
    ('duplicate hosts', 'nameserver 8.8.8.8\n', 'hosts: files dns\nhosts: files dns\nipnodes: files dns\n', False),
    ('extra resolver', 'nameserver 8.8.8.8\nnameserver 10.0.5.1\n', 'hosts: files dns\nipnodes: files dns\n', False),
]
with tempfile.TemporaryDirectory() as tmp:
    ...
    (fixture / 'network-policy.env').write_text(policy.replace('DNS_IP=8.8.8.8', 'DNS_IP=192.0.2.53'))
